/* ============================================================================
   ...
   Architecture
   ------------
   - contentData.js loads first and defines the global `CYSAPLUS` namespace with
...
(function () {
  ...
  var S = window.CYSAPLUS || {};
  /* All exam figures resolve from the single source of truth, CYSAPLUS.exam.
     No exam figure is hard-coded as a numeric literal anywhere in this file. */
  var EX = (S && S.exam) || {};
  var PASS = EX.passing, MINS = EX.minutes, ITEMS = EX.maxQuestions,
      CEIL = EX.scaleHigh, FLOOR = EX.scaleLow, NDOM = EX.domains;
  ...
  var Store = (function () {
    ...
    return {
      ...
      get: function (k, fb) { return read("cysaplus." + k, fb); },
      set: function (k, v) { write("cysaplus." + k, v); },
      del: function (k) { try { localStorage.removeItem("cysaplus." + k); } catch (e) {} }
    };
  })();
  ...
  function renderDashboard() {
    ...
    b1.appendChild(el("p.sectsub", { text: "Start here. Understand how the CS0-004 is scored and delivered, and where the credential fits in an IT/cybersecurity career." }));
    var g1 = el("div.grid");
    g1.appendChild(card({
      icon: "🎓", tag: "OVERVIEW", title: "Exam Mechanics", cls: "",
      desc: "Format, timing, scoring, and question types for the CS0-004 &mdash; the high-stakes details every candidate should internalize before sitting the test.",
      foot: MINS + " min · ≤" + ITEMS + " items · " + PASS + " to pass", cta: "Read",
      onClick: function () { openExamMechanics(); }
    }));
    g1.appendChild(card({
      icon: "🧭", tag: "CAREER", title: "Career Guidance", tagCls: "ghost",
      desc: "Where CySA+ sits on the career ladder: DoD 8140/8570 IAT Level II baseline, and the SOC analyst, sysadmin, and security specialist roles it unlocks.",
    ...
    if (window.FLASH) {
      ...
      gf2.appendChild(card({
        ...
        desc: "Every CySA+ acronym across all " + NDOM + " domains in one deck &mdash; the highest-yield rote material on the exam.",
    ...
    b4.appendChild(el("p.sectsub", { html: "Drawn at random from a bank of <b>" + (S.questions ? S.questions.length : "400+") + "</b> original items, graded with full rationale. Domain quizzes isolate one objective set; the quick quiz mixes all " + NDOM + "." }));
    ...
    var b6 = dashSection("06", "Performance-based question simulators", (S.pbqs ? S.pbqs.length : 30) + " simulations · 6 modules");
    b6.appendChild(el("p.sectsub", { html: "A database of <b>" + (S.pbqs ? S.pbqs.length : 30) + "</b> hands-on PBQs across five formats. Read the exhibit, configure each field, and submit for graded feedback mapped to the objective." }));
    var g4 = el("div.grid");
    ...
    b5.appendChild(el("p.sectsub", { html: "A realistic, full-length simulation of the CS0-004 &mdash; " + ITEMS + " questions weighted to the official domain percentages. Sit it under timed conditions to benchmark your readiness." }));
    var g3c = el("div.grid");
    g3c.appendChild(card({
      icon: "🧪", tag: "MOCK EXAM", title: "Full-Length Mock Exam",
      desc: "A realistic " + MINS + "-minute simulation: " + ITEMS + " questions weighted to the exact domain percentages, with a countdown timer, question flagging, and a domain-by-domain scoring dashboard.",
      foot: ITEMS + " questions · " + MINS + " minutes · timed", cta: "Begin",
    ...
    g5.appendChild(card({
      ...
      desc: "Vetted, free study tools: Professor Messer's full CS0-004 course, the official CompTIA objectives PDF, and trusted community guides.",
  ...
  /* ===========================================================================
     ...
     =========================================================================== */
  function certVer() { var v = window.CYSAPLUS_VER || ""; return v ? "?v=" + v : ""; }
  ...
  function renderCertCatalog(mount) {
    ...
    var search = el("input.cert-search", { type: "text", placeholder: "e.g. CySA+" });
  ...
  function computeReadiness() {
    ...
    var dims = [
      ...
      { key: "mock", label: "Mock exam", weight: 1.6, value: bestMock != null ? Math.round(bestMock / CEIL * 100) : null, has: mocks.length > 0, kind: "score", detail: bestMock != null ? ("best " + bestMock + "/" + CEIL) : "not attempted" },
  ...
  /* Lazily fetch a domain's reading module the first time it's opened, so the
     ...
     script populates CYSAPLUS.reading[id]; once cached it renders instantly. */
  ...
  function loadDomainModule(id, onReady, onFail) {
    ...
    s.src = "assets/js/content/domain" + id + ".js" + (window.CYSAPLUS_VER ? "?v=" + window.CYSAPLUS_VER : "");
    s.async = true;
    s.onload = function () {
  ...
  function openDomain(id) {
    ...
    function render() {
      ...
      Router.replace(readerView("Domain " + id, d.title, "CS0-004 Domain " + id + " · " + d.weight + "% of the exam", sections));
    }
  ...
  function openExamMechanics() { Router.open(readerView("Exam mechanics", "Exam Mechanics: CS0-004", "How the exam is built, delivered, and scored", S.examMechanics || [])); }
  function openCareer() { Router.open(readerView("Career guidance", "Career Guidance", "Where CySA+ fits in your path", S.careerGuidance || [])); }
      ...
      tag: "Analytics", render: function (host) {
        ...
        h.slice(0, 60).forEach(function (r) {
          ...
          var scoreCell = r.mode === "mock"
            ? el("span.pillscore." + (pass ? "pass" : "fail"), { text: r.scaled + " / " + CEIL })
